...
def create_app(sqlite_uri):
    app = Flask(__name__)

    app.secret_key = 'secret-key'
    app.config['SQLALCHEMY_DATABASE_URI'] = sqlite_uri # makes it a file called databse.db
...
def write_user_sqlalchemy(username, password) -> bool:
    ...
    if SUser.query.filter_by(username = username, password=bcrypt.generate_password_hash(password)).first() is not None:
        ...
        db.session.commit()
# def write_post(text, user):
#     newpost = Post(text=text, user_id)
